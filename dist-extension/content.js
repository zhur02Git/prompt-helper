...
function init() {
  setInterval(() => {
    ...
    const wrapperInDOM = document.getElementById("prompt-helper-wrapper");

    // If wrapper disappeared (e.g. Claude.ai rebuilt DOM on new conversation), reset and re-inject
    if (buttonInjected && !wrapperInDOM) {
...
function findInputBox() {
  const selectors = [
    ...
    ".ql-editor[contenteditable='true']",        // Dola
    "[data-testid='chat-input']",                // Gemini alt
    "div[contenteditable='true'].ProseMirror",   // Claude.ai (older)
    "[contenteditable='true'][data-lexical-editor='true']", // Claude.ai (newer, Lexical)
    "div.ProseMirror[contenteditable='true']",   // Claude.ai fallback
...
function watchForSubmit(inputBox, wrapper, setEnhanceMode) {
  ...
  // Method B: watch for input content becoming empty after being non-empty
  // Special handling for Claude.ai which leaves an empty <p><br></p> after submit
  function getEffectiveContent() {
    // For Claude.ai contenteditable: strip out empty <p><br></p> remnants
    const site = detectSite();
    if (site === "claude") {
      return Array.from(inputBox.querySelectorAll("p"))
        .map(p => p.innerText || p.textContent)
        .join("")
        .trim();
    }
    return inputBox.innerText?.trim() || inputBox.value?.trim() || "";
  }

  let lastContent = "";
  const inputObserver = new MutationObserver(() => {
    const currentContent = getEffectiveContent();
    if (lastContent.length > 0 && currentContent.length === 0) {
      // Content just cleared — user submitted
      setTimeout(() => {
        setEnhanceMode();
        snapToInput(inputBox, wrapper);
      }, 300);
  ...
  inputObserver.observe(inputBox, {
    childList: true,
    subtree: true,
    characterData: true,
  });
